...
const DEFAULT_PROCESS_CONFIG: ProcessSectionConfig = {
...
function renderProcessIcon(iconKey: any) {
  if (typeof iconKey === 'string') {
    if (MUI_ICONS[iconKey]) {
      const MuiIcon = MUI_ICONS[iconKey];
      return <MuiIcon sx={{ fontSize: { xs: 25, sm: 29, md: 31 }, color: '#ffffff' }} />;
    }
    if (LUCIDE_ICONS[iconKey]) {
      const LucideIcon = LUCIDE_ICONS[iconKey];
      return <LucideIcon className="w-6 h-6 sm:w-7 sm:h-7 text-white" />;
    }
  }

  if (typeof iconKey === 'function') {
    const CustomIcon = iconKey;
    return <CustomIcon sx={{ fontSize: { xs: 25, sm: 29, md: 31 }, color: '#ffffff' }} />;
  }

  return <EventNoteIcon sx={{ fontSize: { xs: 25, sm: 29, md: 31 }, color: '#ffffff' }} />;
}

export default function ProcessSection() {
  const [sectionData, setSectionData] = useState<ProcessSectionConfig>(DEFAULT_PROCESS_CONFIG);
  ...
  useEffect(() => {
    // 1. Try local storage cache for instant zero-latency paint
    try {
      const cached = localStorage.getItem('digital_spyke_process_steps');
      if (cached) {
        const parsed = JSON.parse(cached);
        if (parsed && Array.isArray(parsed.cards) && parsed.cards.length > 0) {
          setSectionData((prev) => ({ ...prev, ...parsed }));
        }
      }
    } catch (e) {
    ...
    async function fetchProcessData() {
      try {
        ...
        if (json.success && json.data) {
          const freshData: ProcessSectionConfig = {
            badgeText: json.data.badgeText || DEFAULT_PROCESS_CONFIG.badgeText,
            headingPrefix: json.data.headingPrefix ?? DEFAULT_PROCESS_CONFIG.headingPrefix,
            headingHighlight: json.data.headingHighlight || DEFAULT_PROCESS_CONFIG.headingHighlight,
            description: json.data.description || DEFAULT_PROCESS_CONFIG.description,
            ctaText: json.data.ctaText || DEFAULT_PROCESS_CONFIG.ctaText,
            ctaLink: json.data.ctaLink || DEFAULT_PROCESS_CONFIG.ctaLink,
            cards:
              Array.isArray(json.data.cards) && json.data.cards.length > 0
                ? json.data.cards
                : DEFAULT_PROCESS_CONFIG.cards,
          };
          setSectionData(freshData);
          try {
            localStorage.setItem('digital_spyke_process_steps', JSON.stringify(freshData));
          } catch (e) {}
        }
      } catch (err) {
        console.error('Failed to load process steps data:', err);
      }
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            ...
            className="inline-flex items-center justify-center px-4 py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md shadow-xl mb-2 overflow-hidden relative"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 via-cyan-400/20 to-indigo-500/20" />
            {Boolean(sectionData.badgeText && /<[a-z][\s\S]*>/i.test(sectionData.badgeText)) ? (
              <span
                className="relative font-bold tracking-[0.25em] text-white uppercase [&_p]:inline [&_p]:m-0"
                dangerouslySetInnerHTML={{ __html: sectionData.badgeText }}
              />
            ) : (
              <span className="relative font-bold tracking-[0.25em] text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 uppercase">
                {sectionData.badgeText || 'OUR PROVEN PROCESS'}
              </span>
            )}
          </div>
          ...
          <h2
            ...
            className="text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-tight leading-tight text-white m-0 mb-4"
          >
            {Boolean(sectionData.headingPrefix && /<[a-z][\s\S]*>/i.test(sectionData.headingPrefix)) ? (
              <span
          ...
          {Boolean(sectionData.description && /<[a-z][\s\S]*>/i.test(sectionData.description)) ? (
            <div
              style={sectionData.descriptionFontSize ? { fontSize: sectionData.descriptionFontSize } : undefined}
              className="text-sm sm:text-base md:text-lg text-gray-400 leading-relaxed max-w-2xl mx-auto m-0 font-medium [&_p]:m-0 [&_p+p]:mt-2 [&_a]:text-cyan-400 [&_a]:underline [&_strong]:text-white"
              dangerouslySetInnerHTML={{ __html: sectionData.description }}
            />
          ) : (
            <p
              style={sectionData.descriptionFontSize ? { fontSize: sectionData.descriptionFontSize } : undefined}
              className="text-sm sm:text-base md:text-lg text-gray-400 leading-relaxed max-w-2xl mx-auto m-0 font-medium"
            >
        ...
        {steps.map((step, index) => {
          ...
          return (
            <Box
              ...
              sx={{
                position: 'sticky',
                top: { xs: '110px', sm: '150px', md: '290px' },
                zIndex: index + 1,
                paddingTop: '4px',
                paddingBottom: '4px',
                mb: cardGap,
            ...
            >
              <Box
                ...
                sx={{
                  ...
                  height: { xs: 'auto', sm: 'calc(100vh - 340px)', md: 'calc(100vh - 320px)' },
                  minHeight: { xs: '360px', sm: '420px', md: '540px', lg: '580px' },
                  maxHeight: { xs: 'none', md: '680px', lg: '740px' },
                  display: 'flex',
                  flexDirection: 'column',
              ...
              >
                ...
                <Box
                  ...
                  sx={{
                    ...
                    bottom: 0,
                    background:
                      'radial-gradient(ellipse 70% 50% at 20% 0%, rgba(0, 85, 255, 0.16), transparent 75%)',
                    pointerEvents: 'none',
                    zIndex: 1,
                  }}
                />
                ...
                <Typography
                  sx={{
                    ...
                    zIndex: 2,
                    background:
                      'linear-gradient(160deg, rgba(0, 100, 255, 0.45) 0%, rgba(120, 80, 255, 0.18) 50%, rgba(255, 255, 255, 0.04) 100%)',
                ...
                >
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: '14px',
                      boxShadow:
                        '0 8px 30px rgba(0, 85, 255, 0.38), inset 0 1px 2px rgba(255, 255, 255, 0.25)',
                      background:
                        'radial-gradient(circle at center, rgba(0, 153, 255, 0.25) 0%, rgba(8, 11, 24, 0.95) 100%)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    }}
                ...
                >
                  {Boolean(step.title && /<[a-z][\s\S]*>/i.test(step.title)) ? (
                    <motion.div
                      initial={{ opacity: 0, scale: 0.95 }}
                      whileInView={{ opacity: 1, scale: 1 }}
                      transition={{ duration: 0.4 }}
                      style={step.titleFontSize ? { fontSize: step.titleFontSize } : undefined}
                      className="text-2xl sm:text-4xl md:text-6xl font-extrabold tracking-tight text-white drop-shadow-[0_0_25px_rgba(0,153,255,0.3)] transition-all duration-300 text-center m-0 [&_p]:m-0 [&_h1]:m-0 [&_h2]:m-0 [&_h3]:m-0 leading-tight"
                ...
                >
                  <Box
                    sx={{
                      ...
                      mb: 2.5,
                      background:
                        'linear-gradient(90deg, rgba(0, 85, 255, 0) 0%, rgba(0, 85, 255, 0.45) 50%, rgba(0, 85, 255, 0) 100%)',
                    }}
                  />

                  {Boolean(step.description && /<[a-z][\s\S]*>/i.test(step.description)) ? (
                    <div
                      style={step.descriptionFontSize ? { fontSize: step.descriptionFontSize } : undefined}
                      className="text-xs sm:text-sm md:text-base text-gray-300/90 leading-relaxed font-normal text-center mx-auto max-w-[720px] mb-4 sm:mb-5 m-0 [&_p]:m-0 [&_p+p]:mt-2 [&_a]:text-cyan-400 [&_a]:underline [&_strong]:text-white [&_ul]:list-disc [&_ul]:list-inside [&_ol]:list-decimal [&_ol]:list-inside"
                      dangerouslySetInnerHTML={{ __html: step.description }}
                    />
                  ) : (
                    <p
                      style={step.descriptionFontSize ? { fontSize: step.descriptionFontSize } : undefined}
                      className="text-xs sm:text-sm md:text-base text-gray-300/90 leading-relaxed font-normal text-center mx-auto max-w-[720px] mb-4 sm:mb-5 m-0"
                    >
                      {step.description}
                    </p>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 justify-center">
                    {(Array.isArray(step.tags)
                      ...
                      ? (step.tags as string).split(',').map((t) => t.trim()).filter(Boolean)
                      : []
                    ).map((tag, tagIndex) => (
                      <span
                        key={tagIndex}
      ...
      >
        ...
        >
          <Button
            variant="contained"
            href={sectionData.ctaLink || 'contact'}
          ...
          >
            {sectionData.ctaText || 'Work With Us!'}
          </Button>
          <Button
            variant="contained"
